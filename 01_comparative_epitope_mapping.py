# ============================================
# SARS-CoV-2 Epitope Analysis Pipeline (CD8 T Cell Epitopes)
Integrated Analysis Pipeline for SARS-CoV-2 CD8+ T Cell Epitopes
# Analyses included the following operations across SARS-CoV-2 variants:
1. CD8+ T Cell Epitope Overlap
...
# ============================================
# Analysis 1: Epitope Overlap (UpSet + Heatmap)
# ============================================
...
# ============================================
# Analysis 2: Epitope Preservation vs Ancestral
# ============================================
...
# ============================================
# Analysis 3: Shared Mutations Across Variants
# ============================================
...
# ============================================
# Analysis 4: Impact of Variant-Defining Mutations on Ancestral Epitopes
# ============================================
def analyze_variant_defining_mutations(epitope_file, mutation_file, output_excel="epitope_mutation_status.xlsx"):
    ...
    plot = plot_data.plot(kind="bar", stacked=True, ax=ax, color=["#194885", "#57BEC1"], edgecolor="white")

    # Add labels inside the bars by iterating through the plot patches
    # Access data directly from results_df using index and column names
    for i in range(len(results_df)):
        unmut_count = results_df.loc[i, "Unmutated Count"]
        mut_count = results_df.loc[i, "Mutated Count"]
        unmut_pct = results_df.loc[i, "Unmutated %"]
        mut_pct = results_df.loc[i, "Mutated %"]

        # Position of unmutated label
        ax.text(i, unmut_pct/2, str(unmut_count), ha="center", va="center", color="white", fontsize=10, fontweight="regular")
        # Position of mutated label
        ax.text(i, unmut_pct + mut_pct/2, str(mut_count), ha="center", va="center", color="white", fontsize=10, fontweight="regular")

    ax.set_ylabel("Percentage of Epitopes (%)")
    ax.set_title("Impact of Variant-Defining Mutations on Ancestral Spike CD8+ T Cell Epitopes")
    ...
    # ----------------------------
    # 4b. Heatmap: Epitope-Relevant Mutations
    # ----------------------------
    all_mutations = []
    for v in variants:
...
# ============================================
# Analysis 6: Positional Mapping of Epitopes
# ============================================
...
if __name__ == "__main__":
    ...
    analyze_epitope_overlap("CD8_variant_epitopes.xlsx", include_ancestral=True)

    # Epitope preservation and variant-specific
    analyze_epitope_preservation("CD8_variant_epitopes.xlsx", show_unmatched=True)

    # Shared mutations across variants
    analyze_shared_mutations("variant_mutations.xlsx")

    # Impact of variant-defining mutations
    analyze_variant_defining_mutations("CD8_epitopes.xlsx", "variant_mutations.xlsx")

    # New positional mapping
    analyze_positional_mapping("CD8_epitopes.xlsx", "CD8_variant_epitopes.xlsx", "variant_mutations.xlsx")
