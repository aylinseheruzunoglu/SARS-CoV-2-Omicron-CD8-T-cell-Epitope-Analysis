...
# %pip install upsetplot pandas matplotlib seaborn openpyxl
...
STRONG_BINDER_THRESHOLD = 1
PERCENTILE_COL = "netmhcpan_el percentile"
PEPTIDE_COL = "peptide"
ALLELE_COL = "allele"
START_COL = "start"
END_COL = "end"
...
for allele_counts in allele_dfs:
    ...
    ax = counts.plot(kind="bar", color="#007994")
    plt.title(f"Distribution of Strong-Binding Epitopes per HLA Class I Allele\n{variant}", fontsize=14)
    plt.xlabel("HLA Class I Allele", fontsize=12)
...
plt.title("Comparison of Strong-Binding Epitope Counts Across Variants per HLA-I Allele", fontsize=14)
plt.xlabel("HLA Class I Allele", fontsize=12)
plt.ylabel("Number of Unique Epitopes", fontsize=12)
plt.xticks(rotation=90)
...
# Add a check here to prevent plotting an empty DataFrame
if not heatmap_data.empty:
    ...
    print("\nNo peptide–HLA pairs with mutation overlap across variants to plot heatmap.")
